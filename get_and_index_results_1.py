...
from elastic_meth_1 import Elastic_meth
...
def generate_report(endpoint, more_res, l_graph, dis_res, invalid_json, partition) -> None:
    ...
    if report_query_graph == {}:
        ...
        if len(more_res) == 2:
            note.write(Latex.format_special_char(f"The {query} returns more results ({more_res[0]})\n"))
    ...
    note.write(Latex.section("Partition in separated queries"))
    if partition is True:
        ...
        note.write(f"The volume of {dis_res} results does not need query partition\n")
    ...
    # returns empty and null values
        ...
        if isinstance(e, SPARQLExceptions.EndPointInternalError) or \
            ...
            if q.search_graph is False:
                ...
                key = Latex.format_special_char(f"Error on  {q.name} : ")
            if len(errors) == 1:
                value = Latex.format_special_char(f"{errors}")
            else:
...
def main():
    ...
    for j in range(8, 10):
        ...
        # converting distinct set of string results into a json array of values
        for r in distinct_res:
            try:
                json_obj = json.loads(r)
                json_array = extracting_values_from_result(json_obj)
                values_list.append(json_array)
            except Exception as e:
                print(e)
    ...
    # report - about timings and closing the "report.tex" document
    note = open("report.tex", "a", encoding="utf-8")
